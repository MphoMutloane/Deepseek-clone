...
const Message = ({role, content}) => {
  return <div className="flex flex-col items-center w-full max-w-3xl text-sm">
    <div className={`flex flex-col w-full mb-8 ${role === "user" && "items-end"}`}>
        <div className={`group relative flex max-w-2xl py-3 rounded-xl ${role === "user" ? "bg-[#414158] px-5": "gap-3"}`}>
            ...
            </div>
            {
                ...
                (
                    <>
                    <Image src={assets.logo_text} alt="" className="h-9 w-9 p-1 border border-white/15 rounded-full" />
